...
(function () {
  ...
  function pointOnLand(lat, lon) {
    ...
    const qlat = ((lat * 100) | 0), qlon = ((lon * 100) | 0);
    const k = qlat * 100000 + qlon;
    const hit = memo.get(k);
    if (hit !== undefined) return hit;
    // Evaluate AT the quantized point, not the raw one: the memo used to
    // cache whichever raw point hit a 0.01° cell first, so answers near
    // coastlines depended on query HISTORY (route members were coupled
    // through the cache). Quantized evaluation makes this a pure function —
    // order-independent, and exactly reproducible by the Rust engine.
    const v = pointOnLandRaw(qlat / 100, qlon / 100);
  ...
  function segmentClear(p1, p2) {
    ...
    const dLonNm = (p2.lon - p1.lon) * 60 * FdMath.cos((p1.lat + p2.lat) * 0.5 * Math.PI / 180);
  ...
  async function buildRaster(w, s, e, n, res = 0.02, onProgress) {
    ...
    return {
      ...
      // dilation belt can never be legally approached. `acceptable` lets the
      // caller add constraints: the router's hand LAND boxes treat whole bays
      // as land, and an anchor that is raster-water but in-box strands the
      // fleet (zero legal legs — seen live: Newport snapped into the
      // Sakonnet box and every member died at the dock).
      nearestWater(lat, lon, maxNm = 15, acceptable) {
        const ok = (la, lo) => !rasterPoint(la, lo) && (!acceptable || acceptable(la, lo));
        if (ok(lat, lon)) return { lat, lon };
        for (let r = 1; r <= maxNm; r++) {
          for (let b = 0; b < 360; b += 20) {
            const p = {
              lat: lat + (r / 60) * FdMath.cos(b * Math.PI / 180),
              lon: lon + (r / (60 * FdMath.cos(lat * Math.PI / 180))) * FdMath.sin(b * Math.PI / 180),
            };
            if (ok(p.lat, p.lon)) return p;
          }
        }
        return null;
      },
