...
(function () {
  const VIEWS = [
    ...
    { href: "helm.html",    icon: "🧭", label: "Helm" },
  ...
  .mclock .d{font-size:9px;color:#9ba8bd;letter-spacing:0.1em;margin-top:2px;cursor:pointer;line-height:1}
  .mclock .d:hover{color:#e8ecf3}
  ...
  function makeClock() {
    ...
    wrap.innerHTML = `<div class="t" id="mclockTime">--:--:--</div>` +
      `<div class="d" id="mclockDate" title="Click to switch date format">--/--/----</div>` +
      `<div class="z" id="mclockZone">ZULU ▾</div>` +
  ...
  const DATEFMT_KEY = "meridian.clock.datefmt"; // intl | long | us
  const dateFmt = () => { try { return localStorage.getItem(DATEFMT_KEY) || "intl"; } catch (e) { return "intl"; } };
  function fmtDate(tz) {
    const now = new Date();
    try {
      switch (dateFmt()) {
        case "long": return new Intl.DateTimeFormat("en-US", { timeZone: tz, month: "long", day: "numeric", year: "numeric" }).format(now);
        case "us": return new Intl.DateTimeFormat("en-US", { timeZone: tz, month: "2-digit", day: "2-digit", year: "2-digit" }).format(now);
        default: return new Intl.DateTimeFormat("en-GB", { timeZone: tz, day: "2-digit", month: "2-digit", year: "numeric" }).format(now);
      }
  ...
  function renderClockTime() {
    ...
    const dEl = document.getElementById("mclockDate");
    if (dEl) dEl.textContent = fmtDate(tz);
  ...
  function initClock() {
    ...
    if (dEl) dEl.addEventListener("click", (ev) => {
      ev.stopPropagation(); // the date cycles formats; it doesn't open the TZ list
      const order = ["intl", "long", "us"];
      const next = order[(order.indexOf(dateFmt()) + 1) % order.length];
      try { localStorage.setItem(DATEFMT_KEY, next); } catch (e) { /* noop */ }
      renderClockTime();
    });
