...
(function () {
  ...
  function routeMemberIso(memberInfo, env) {
    ...
    const mkNode = (lat, lon, time, parent, heading, bsp, tws, twa, currAlong) =>
      ({ lat, lon, time, parent, heading, bsp, tws, twa, currAlong });
    ...
    for (let t = 0; t < MAX_H && front.length && !arrival; t += DT) {
      ...
      const consider = (node) => {
        if (!segmentClear(node.parent || node, node)) return false;
        ...
        // Rank by projection onto the start→dest axis, NOT raw distance from
        // start: raw distance rewards riding a fast current (Gulf Stream) dead
        // sideways forever, which produced grotesque around-the-houses tracks.
        const score = dist * Math.cos(bs * D2R);
        const cur = buckets.get(key);
        if (!cur || score > cur.score) buckets.set(key, { node, score });
      ...
      for (const pt of front) {
        ...
        if (wind.speed < 0.1) {
          // Becalmed: park (drift with current if any) rather than killing the branch.
        ...
        // Direct-shot arrival check: if the boat can fetch the destination within
        // this step on the straight bearing, record a sub-step ETA.
        {
          ...
          if (bsp > 0.2) {
            ...
            if (ogToDest > 0.3 && dPtDest <= ogToDest * DT + TOL && segmentClear(pt, dest)) {
        ...
        if (!expanded) {
          // Whole fan stalled (no-go cone + land wall) — park the branch and
          // wait for the wind to shift instead of killing it.
    ...
    const path = [];
    let maxTws = 0, sumTws = 0, twsCount = 0, maxFav = 0, maxAdv = 0;
    for (let n = endNode; n; n = n.parent) {
      path.push({ lat: n.lat, lon: n.lon, time: n.time, bsp: n.bsp, tws: n.tws, twa: n.twa, heading: n.heading, curr_kt: Math.abs(n.currAlong) });
      if (n.parent) {
