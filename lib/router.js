...
(function () {
  ...
  function gcDistance(p1, p2) {
    ...
    const a = FdMath.sin(dlat / 2) ** 2 + FdMath.cos(lat1) * FdMath.cos(lat2) * FdMath.sin(dlon / 2) ** 2;
  ...
  function fastDistNm(p1, p2) {
    ...
    const dx = (p2.lon - p1.lon) * FdMath.cos((p1.lat + p2.lat) * 0.5 * D2R);
    return 60 * Math.sqrt(dx * dx + dy * dy);
  }
  function bearing(p1, p2) {
    const lat1 = p1.lat * D2R, lat2 = p2.lat * D2R, dlon = (p2.lon - p1.lon) * D2R;
    const y = FdMath.sin(dlon) * FdMath.cos(lat2);
    const x = FdMath.cos(lat1) * FdMath.sin(lat2) - FdMath.sin(lat1) * FdMath.cos(lat2) * FdMath.cos(dlon);
    return (FdMath.atan2(y, x) * R2D + 360) % 360;
  }
  function sphereWalk(pos, hdgDeg, distNm) {
    const lat1 = pos.lat * D2R, lon1 = pos.lon * D2R, hdg = hdgDeg * D2R, d = distNm / EARTH_R_NM;
    const lat2 = Math.asin(FdMath.sin(lat1) * FdMath.cos(d) + FdMath.cos(lat1) * FdMath.sin(d) * FdMath.cos(hdg));
    const lon2 = lon1 + FdMath.atan2(FdMath.sin(hdg) * FdMath.sin(d) * FdMath.cos(lat1), FdMath.cos(d) - FdMath.sin(lat1) * FdMath.sin(lat2));
  ...
  function windAt(lat, lon, hourIdx, memberInfo, windResults) {
    ...
    for (let i = 0; i < windResults.length; i++) {
      ...
      totalWeight += w;
      const dr = dir * D2R;
      u += -speed * FdMath.sin(dr) * w; v += -speed * FdMath.cos(dr) * w;
    }
    if (!totalWeight) return { speed: 0, dir: 0 };
    u /= totalWeight; v /= totalWeight;
    return { speed: Math.sqrt(u * u + v * v), dir: (FdMath.atan2(-u, -v) * R2D + 360) % 360 };
  }
  ...
  function currentAt(lat, lon, hourIdx, marineResults) {
    ...
    for (const cr of marineResults) {
      ...
      totalWeight += w;
      const dr = dt * D2R;
      u += sp * FdMath.sin(dr) * w; v += sp * FdMath.cos(dr) * w;
    }
    if (!totalWeight) return { speed_kt: 0, dir_to_deg: 0 };
    u /= totalWeight; v /= totalWeight;
    const kmh = Math.sqrt(u * u + v * v);
    return { speed_kt: kmh * 0.539957, dir_to_deg: (FdMath.atan2(u, v) * R2D + 360) % 360 };
  }
  ...
  function routeMemberIso(memberInfo, env) {
    ...
    for (let t = 0; t < MAX_H && front.length && !arrival; t += DT) {
      ...
      // sidecar route/engine.rs — change together.)
      const hourIdx = Math.min(Math.floor(t + depOffsetHours), flen - 1);
      ...
      for (const pt of front) {
        ...
        const cx = curr.speed_kt * FdMath.sin(curr.dir_to_deg * D2R), cy = curr.speed_kt * FdMath.cos(curr.dir_to_deg * D2R);
        ...
        const emitMotor = () => {
          for (const dh of [0, -25, 25, -45, 45]) {
            ...
            if (curr.speed_kt > 0.02) {
              ...
              currAlong = curr.speed_kt * FdMath.cos(circDiff(curr.dir_to_deg, h) * D2R);
            }
          ...
          const vx = motorKt * FdMath.sin(bTo * D2R) + cx, vy = motorKt * FdMath.cos(bTo * D2R) + cy;
          const ogToDest = vx * FdMath.sin(bTo * D2R) + vy * FdMath.cos(bTo * D2R);
        ...
        // within this step on the straight bearing, record a sub-step ETA.
        {
          ...
          if (bsp > 0.2) {
            const vx = bsp * FdMath.sin(bTo * D2R) + cx, vy = bsp * FdMath.cos(bTo * D2R) + cy;
            const ogToDest = vx * FdMath.sin(bTo * D2R) + vy * FdMath.cos(bTo * D2R); // closing speed
            if (ogToDest > 0.3 && dPtDest <= ogToDest * DT + TOL && legClear(pt, dest)) {
              const eta = t + Math.min(DT, dPtDest / ogToDest);
              if (!arrival || eta < arrival.eta)
        ...
        for (let dh = -HEAD_FAN; dh <= HEAD_FAN; dh += HEAD_STEP) {
          ...
          if (consider(cand)) {
            ...
            const vx = bsp * FdMath.sin(h * D2R) + cx, vy = bsp * FdMath.cos(h * D2R) + cy;
            const vmg = vx * FdMath.sin(bTo * D2R) + vy * FdMath.cos(bTo * D2R);
