...
// Set VITE_DEM_TILE_URL to a single mosaic/tilejson template, e.g.
// http://localhost:8081/cog/tiles/WebMercatorQuad/{z}/{x}/{y}@1x?url=...&rescale=0,9000&nodata=-9999
// If unset, terrain stays off (prevents hammering with 404/500).
const DEM_TILE_URL = import.meta.env.VITE_DEM_TILE_URL || null;
...
async function findDemUrl(lat, lon) {
  if (DEM_TILE_URL) return { url: DEM_TILE_URL, reason: "env override" };
  const offsets = [
    [0, 0],
    [1, 0],
    [-1, 0],
  ...
  for (const [dy, dx] of offsets) {
    ...
    const sample = candidate
      ...
      .replace("{x}", String(x))
      .replace("{y}", String(y));
    const ok = await headOk(sample, 2000);
    if (ok) return { url: candidate, reason: `cog @ (${Math.floor(lat+dy)},${Math.floor(lon+dx)})` };
  }
  return null;
}
...
const Chart = () => {
  ...
  useEffect(() => {
    ...
    const demTileUrl = DEM_TILE_URL || buildDemTileUrl(data.lat, data.lon);
    setDemDebug(demTileUrl ? `DEM candidate: ${demTileUrl}` : "DEM off (no URL)");
    ...
    map.on("load", () => {
      ...
      redrawGrid();
      if (demTileUrl && map.getSource("dem")) {
